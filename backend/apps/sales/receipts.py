...
def render_pdf_receipt(sale) -> bytes:
    ...
    def ps(name, **kw):
        defaults = dict(fontName="Helvetica", fontSize=8, leading=11, alignment=TA_LEFT)
        defaults.update(kw)
    ...
    s_tagline = ps("tg", fontName="Helvetica-Oblique", fontSize=7.5, leading=10,
    ...
    s_dev     = ps("dv", fontName="Helvetica-Oblique", fontSize=7, leading=10,
                   alignment=TA_CENTER, textColor=LGRAY)
    s_devbold = ps("db", fontName="Helvetica-Bold", fontSize=7.5, leading=10,
                   alignment=TA_CENTER, textColor=GRAY)
    ...
    if items_disc_total:
        totals_rows.append([
            Paragraph(f"Gross subtotal ({len(items)} item{'s' if len(items) != 1 else ''})", s_tlbl),
    ...
    if pay_row_idx < len(totals_rows):
        style_cmds.append(
            ("LINEABOVE", (0, pay_row_idx), (-1, pay_row_idx), 0.3, LGRAY)
        )
